...
async function loadModels() {
  ...
  const BLOCKED_MODELS = [
    'north-mini-code-free', 'claude-sonnet-5',
    'kimi-k2.7-code', 'minimax-m3', 'grok-4.5', 'gpt-5.6-sol', 'gpt-5.6-terra',
    ...
    'claude-opus-4-5', 'claude-opus-4-1', 'claude-sonnet-4-6', 'claude-sonnet-4-5',
  ...
  options.innerHTML =
    ...
    renderFolder('Custom', customItems) +
    renderFolder('Cloud', cloudItems) +
    renderFolder('Local', localItems)
    ...
    const cmds = {
      ...
      'context-window': () => {
        ...
        if (value) {
          ...
          showNotification(`Context Window: ${value}`, 'info')
        }
      },
      'context-window-info': () => {
        const used = args[0] || '0'
        ...
        showNotification(`Context Window: ${used} (${pct}%)`, 'info')
      },
      'reserved': () => {
...
if (cmds[name]) cmds[name]()
  ...
  const renderCreditMenu = () => {
    ...
    if (contextEl) {
      ...
      const contextUsed = messages.length
      const formatTokens = (n) => {
        if (n >= 1000000) return `${(n / 1000000).toFixed(1)}M`
        if (n >= 1000) return `${(n / 1000).toFixed(1)}K`
        return String(n)
      }
      contextEl.textContent = `${formatTokens(contextUsed)}`
    }
    const contextBarEl = document.querySelector('[data-context-bar]')
    const contextPctEl = document.querySelector('[data-context-pct]')
    if (contextBarEl && contextPctEl) {
      const contextMax = 200000
      const contextUsed = messages.length
      const contextPct = Math.min(100, Math.round((contextUsed / contextMax) * 100))
      contextBarEl.style.width = `${contextPct}%`
      contextBarEl.classList.toggle('low', contextPct >= 75 && contextPct < 100)
      contextBarEl.classList.toggle('empty', contextPct >= 100)
      contextPctEl.textContent = `${contextPct}%`
    }
  }
  ...
  document.querySelector('[data-credit-toggle]')?.addEventListener('click', (e) => {
    ...
    creditMenu.hidden = !creditMenu.hidden
    if (!creditMenu.hidden) renderCreditMenu()
  })
  ...
  window.electron?.getContextWindowInfo?.().then((info) => {
    if (info?.ok) {
      contextWindowBoostEnabled = false
      const contextMaxEl = document.querySelector('[data-context-max]')
      if (contextMaxEl && info.maxTokens) {
        contextMaxEl.textContent = `${Math.floor(info.maxTokens / 1000)}K`
      }
    }
  })
  ...
  form?.addEventListener('submit', async (e) => {
    ...
    const setStreaming = (on) => {
      ...
      // Add/remove loading class to the composer element for visual feedback
    ...
    try {
      ...
      if (false) {
        ...
        const payload = isDeepResearch
          ? { url: route.url, model, topic: researchQuery, provider: route.provider, apiKey }
          : { url: route.url, model, query: researchQuery, provider: route.provider, apiKey }
  ...
  const lastSession =
    sessions.find((s) => Array.isArray(s.messages) && s.messages.length > 0) || sessions[0]
  if (lastSession) {
